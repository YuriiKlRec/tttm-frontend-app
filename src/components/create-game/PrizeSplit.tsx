  ...
  return (
    <div className="flex w-full flex-col items-center gap-1.5 bg-[rgba(255,255,255,0.05)] py-3">
      <div className="flex w-full gap-3 px-7 py-2">
        <Column icon={starIcon} label="Host (You):" percent={formatPercent(split.host)} />
        <Column icon={rocketIcon} label="Platform:" percent={formatPercent(split.platform)} muted />
        <Column icon={trophyIcon} label="Winner:" percent={formatPercent(split.winner)} muted />
      </div>
      ...
      <span className="text-center font-body text-[12px] text-text-secondary">
        Host break-even point: ~{breakEven ?? '—'} predictions in game
      </span>
    </div>
  )
}
