  ...
  <li className="flex items-center justify-between px-7">
    ...
    {status === 'taken' ? (
      <div className="flex items-center gap-3">
        <span className="font-mono text-[13px] font-bold text-[#e5484d]">Already taken</span>
        <button
          type="button"
          aria-label={`Why is ${formatUsd(price)} taken`}
          onClick={onInfo}
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          ...
          aria-label={
            status === 'active'
              ? `Skip prediction ${formatUsd(price)}`
              : `Restore prediction ${formatUsd(price)}`
          }
          onClick={onToggle}
          className="flex h-4 w-4 items-center justify-center outline-none"
        >
